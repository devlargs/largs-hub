...
| 📥 **Download handling**    | Choose a download folder, and optionally open the file or folder and get an alert when a download finishes, with a link that shows the file in File Explorer (Windows) or Finder (macOS). Only documents, images, audio and video open automatically; programs, scripts, installers and archives are shown in their folder instead. |
...
| ⬆️ **Auto-updates**         | Checks GitHub Releases, then downloads the latest version, verifies it against the sha256 checksum GitHub publishes, installs it and reopens the app, on Windows and macOS. An update with no checksum is never installed; it links to its GitHub page to download by hand.                                                         |
