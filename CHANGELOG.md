...
- **Behind the scenes: the largest files split into smaller pieces.** The Messenger automation panel, the Settings page, the Add service dialog, the app shell, the main process entry point and the Messenger automation scheduler were broken into focused parts, and their form, update-status, add-service, keyboard-shortcut, window-size saving, overlay and scheduling logic now has automated tests. Nothing should look or behave differently on Windows or macOS.
