...
- **Behind the scenes: the largest files split into smaller pieces.** The Messenger automation panel, the Settings page, the Add service dialog, the app shell, the main process entry point, the Messenger automation scheduler, the updater, the service IPC handlers and the saved message lists picker were broken into focused parts, and their form, update-status, add-service, keyboard-shortcut, window-size saving, overlay, scheduling, release-picking, service-list and list search and paging logic now has automated tests. Nothing should look or behave differently on Windows or macOS. For contributors: `npm run build` and `npm run dev` now clear `dist-electron/` first, so a file left over from an older build can't be loaded in place of the current one.
