...
- **Behind the scenes: the largest files split into smaller pieces.** The Messenger automation panel, the Settings page, the Add service dialog, the app shell, the main process entry point, the Messenger automation scheduler, the updater and the service IPC handlers were broken into focused parts, and their form, update-status, add-service, keyboard-shortcut, window-size saving, overlay, scheduling, release-picking and service-list logic now has automated tests. Nothing should look or behave differently on Windows or macOS. For contributors: `npm run build` and `npm run dev` now clear `dist-electron/` first, so a file left over from an older build can't be loaded in place of the current one.
