...
Each service view gets: a desktop-Chrome identity for the OS it's running on (sites like Google/WhatsApp reject Electron). `userAgent.ts` builds the UA string, the `Sec-CH-UA*` headers and the `navigator.userAgentData` metadata from one pure function, and `chromeIdentity.ts` applies them: the session's header rewrite, plus Chromium's own UA override via `webContents.debugger` (`Emulation.setUserAgentOverride`), set before the first `loadURL` so the page never sees Electron's values. Any new `webContents` that loads service pages (a popup window, a preview) needs `applyChromeIdentity` / `loadWithChromeIdentity` too. Google's sign-in page rejects that Chrome anyway (it checks for Chrome-only page APIs Electron lacks), so while a view's top-level page is on `accounts.google.com` it presents as Firefox instead (`signInIdentity.ts`, switched per main-frame navigation in `chromeIdentity.ts`). The switch has to happen before a navigation starts: a server redirect that crosses onto or off the sign-in page is cancelled by `navigationGuard.ts` and restarted with `loadWithIdentityFor`, and nothing may call `setUserAgent()` mid-navigation (Chromium reloads the page, which loops). It also gets notification-count detection (title-pattern `(N)` plus per-service DOM polling via `executeJavaScript`, with decreases debounced in `notificationCounts.ts` to avoid badge flicker), Ctrl+1-9 service switching intercepted in `before-input-event`, and a `setWindowOpenHandler` that navigates in-view for the service's own domain / an allowlist of auth domains and ignores other http(s) links (they open via the "View Link" preview).

Messenger automation is split the same way in `electron/messengerAutomation/`: `index.ts` (IPC handlers + public API), `runtime.ts` (deps, task list, push/inject helpers), `tasks.ts` (the scheduler), `callCycle.ts`, `lifecycle.ts` (create/stop), `restore.ts`, `autoStop.ts`, and the pure `schedule.ts`, `notice.ts`, `validation.ts` and `scripts.ts`.
